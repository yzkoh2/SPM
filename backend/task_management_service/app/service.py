from .models import db, Task, Subtask, Comment, Attachment, TaskCollaborator
...
def update_task(task_id, task_data):
    """Update an existing task - COMPLETE VERSION"""
    try:
...
def create_subtask(task_id, subtask_data):
    ...
    try:
        ...
        if subtask_data.get('deadline'):
            try:
                if isinstance(subtask_data['deadline'], str) and subtask_data['deadline'].strip():
                    deadline_str = subtask_data['deadline']
                    if 'T' in deadline_str:
                        deadline = datetime.fromisoformat(deadline_str.replace('Z', '+00:00'))
                    else:
                        deadline = datetime.strptime(deadline_str, '%Y-%m-%d %H:%M:%S')
                elif isinstance(subtask_data['deadline'], datetime):
...
def update_subtask(task_id, subtask_id, subtask_data):
    """Update an existing subtask - COMPLETE VERSION"""
    try:
        subtask = Subtask.query.filter_by(id=subtask_id, task_id=task_id).first()
        if not subtask:
            return None
