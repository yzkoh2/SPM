<template>
    <div>
        ...
        <!-- Empty State -->
        <div v-else-if="filteredReports.length === 0" class="text-center py-12 bg-white shadow rounded-lg">
              ...
              <div class="px-6 py-4">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                      <span
                        :class="[
                          ...
                          report.report_type === 'project' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                        ]"
                      >
                  ...
                  <div class="ml-4 flex items-center gap-2">
                    ...
                    >
                      <svg v-if="downloading !== report.id" class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
                      </svg>
                      <div v-else class="animate-spin rounded-full h-4 w-4 border-b-2 border-gray-700 mr-1"></div>
                      Download
                    </button>
                ...
                <div class="sm:flex sm:items-start">
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div class="mt-2">
                      ...
                      <p class="mt-2 text-sm font-medium text-gray-700">{{ reportToDelete?.file_name }}</p>
                    </div>
                  </div>
                </div>
              </div>
  ...
  const loadReports = async () => {
    ...
    try {
      const response = await fetch(`${KONG_API_URL}/reports/history?user_id=${authStore.user.id}&limit=100`)
  ...
  const downloadReport = async (reportId, fileName) => {
    ...
    try {
      const response = await fetch(`${KONG_API_URL}/reports/history/${reportId}/download?user_id=${authStore.user.id}`)
  ...
  const deleteReport = async () => {
    ...
    try {
      const response = await fetch(`${KONG_API_URL}/reports/history/${reportToDelete.value.id}?user_id=${authStore.user.id}`, {
        method: 'DELETE'
      })
