<template>
  <div v-if="show" class="fixed inset-0 z-50 overflow-y-auto" @click.self="$emit('close')">
    ...
    >
      ...
      <div
        class="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg relative z-10"
      >
        ...
        <form @submit.prevent="saveProjectChanges" class="space-y-6">
          ...
          <div class="border-t pt-6">
            ...
            <p class="text-xs text-gray-500 mt-1">
              Currently owned by:
              {{ ownerDetails ? ownerDetails.name : `User ${project?.owner_id}` }}.
            </p>
          </div>
          ...
          <div class="border-t pt-6">
            ...
            >
              <div
                v-if="collaboratorDetails.length === 0"
              ...
              >
                ...
                <button
                  ...
                  :disabled="!isOwner"
                  class="text-red-500 hover:text-red-700 text-xs font-semibold p-1 rounded-full hover:bg-red-50 transition disabled:text-gray-400"
                >
...
const removeCollaborator = (collaboratorIdToRemove) => {
  ...
  if (
    !confirm(
      `Remove this user from the project? This change will be saved when you click 'Save Changes'.`,
    )
  )
    return

  localCollaborators.value = localCollaborators.value.filter((id) => id !== collaboratorIdToRemove)
  collaboratorDetails.value = collaboratorDetails.value.filter(
    (detail) => detail.user_id !== collaboratorIdToRemove,
  )
